...
## Asserting board/label geometry (narrow-width 3D layout)

Layout regressions here are internal to the board panel: the panels can sit happily inside the
viewport while the grid itself escapes its own panel. So `scrollWidth == clientWidth` and
"panel inside viewport" are **not** sufficient — measure against the panel's padding box.

The structure (built in `src/ui.ts`, styled at the bottom of `src/styles.css`) is:

```
.board-wrap                      <- the panel; border + padding, 3D-only background
  .board-frame                   <- grid: `1.2rem auto` columns/rows
    .coordinate-labels-top       <- col 2 / row 1, letters A-J
    .coordinate-labels-side      <- col 1 / row 2, numbers 1-10
    .board                       <- col 2 / row 2, the 100 cell buttons
```

Useful invariants to assert, per board (`#player-board` and `#ai-board`):

- Width algebra: `.board` width = `10*cell + 9*gap + 2*2px padding`;
  `.coordinate-labels-top` width = `10*cell + 9*gap + 2*2px padding`. **The two gaps must match**,
  otherwise the label track is wider than the grid, which both widens `.board-frame` past the panel
  and drifts every letter rightwards (drift grows with the column index — check column J, not A).
  In 3D both gaps are `1px`; in Classic both are `2px`. So `labelsTop.width - board.width == 0`
  is a single sharp assertion for either view.
- Containment: compute the wrap's padding box from `getComputedStyle` border+padding and assert the
  frame, board, both label tracks, the `J` span and the `10` span all fall inside it.
- Alignment: compare `centre(labelsTop.children[i])` with `centre(cells[i])` for i in 0..9, and
  `centre(labelsSide.children[i])` with `centre(cells[i*10])`. Expect 0px drift.
- The label tracks are `aria-hidden="true"`, so they never appear in the accessibility tree —
  assert them from rects plus a screenshot, never from DOM text.

3D `--cell` is width-derived on mobile and clamped on desktop, so expected cell sizes are worth
precomputing before measuring (a mismatch tells you which rule actually applied):
`@media (max-width: 700px)` is **inclusive**, so 700px uses the mobile rule.

## Pre-game flow: campaign cards → briefing → deploy

Fresh loads open on the home/command screen (`#command-screen`), which is a battle *chooser*, not a
settings form. There is no DEPLOY FLEET button there any more.

- Cards: `#campaign-cards [data-theatre="trafalgar|salamis|midway"]`. Each card is a **single
  `<button>`** (the TAKE COMMAND pill is a styled `<span>`, not a nested button), so both `Enter` and
  `Space` must activate it — test both, since a nested button would break one.
- Clicking a card sets `session.theatre`, calls `director.setTheatre()` immediately and shows
  `#briefing-screen`. Briefing content: `#briefing-art`, `#briefing-label`, `#briefing-years`,
  `#briefing-enemy`, `#briefing-intro-1..3`, `[data-difficulty]`, `[data-mode]`,
  `[data-action='begin-battle']`, `#back-to-command`.
- **The discriminating test for cross-wiring** is to open a NON-default card first (e.g. `salamis`,
  since `trafalgar` is the default theatre) and assert every field, including all three intro lines.
  Card copy lives in `THEATRES` (`src/three/theatres.ts`) — compare against it, not against
  hardcoded strings.
- `BEGIN BATTLE` only sets `screen = "deploy"`; it does **not** call `resetGame`, so difficulty/mode
  picked on the briefing must survive into the battle. Verify from
  `localStorage["battleship.session.v1"]` (`difficulty`/`mode`) *and* visually (`#salvo-bar` visible
  with "Targets n/5"; CMDR Voss greets you with the difficulty posture, e.g. "Admiral posture").
- Route table (`src/main.ts`), worth re-checking after any flow change:
  `#back-to-command` → `command`; `#new-game` → `command`; `#new-battle` → `command`;
  `#rematch` → `deploy`; `#change-difficulty` → `briefing` **and** focuses `difficultyButtons[0]`
  (RECRUIT). `resetGame` preserves `session.theatre`, so the briefing keeps the battle just played.
- Restore never yields a pre-game screen: `saved.phase` maps `placement→deploy`, `playing→battle`,
  otherwise `debrief`. To test a refresh *mid-enemy-turn*, fire and reload immediately — confirm
  `#turn-banner` reads "ENEMY ANALYZING" and saved `turn === "ai"` before reloading. The pending AI
  shot resolves after the reload (log gains a line); the screen stays `battle`.
- Mobile: `.campaign-cards` collapses to one column at `max-width: 700px`. `.briefing-art` uses a
  deliberate negative-margin bleed (`width: calc(100% + 1.8rem)`), so check
  `documentElement.scrollWidth === clientWidth` at 320px **and** 280px, and that BEGIN BATTLE /
  back resolve to themselves under `document.elementFromPoint`.

## Driving a game fast

- Fresh loads open on the home screen: click a campaign card, then **BEGIN BATTLE**, then skip manual
...
- **Do not sample the WebGL canvas via `drawImage`/`getImageData`** — the context has no
  `preserveDrawingBuffer`, so readback is all-black `(0,0,0)`. To compare theatre sea colours, take a
  page screenshot (which composites correctly) and sample the PNG offline (e.g. PIL). Salamis is
  teal (green ≫ red), Trafalgar/Midway are blue-dominant, so a G−R comparison discriminates them.
