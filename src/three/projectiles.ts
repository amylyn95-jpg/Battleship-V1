...
export function createProjectile(kind: ProjectileKind, from: ArcPoint, to: ArcPoint, now: number): ProjectileFlight {
  ...
  if (kind === "cannonball") {
    const cannonball = mesh(new THREE.SphereGeometry(1.6, 10, 8), new THREE.MeshStandardMaterial({ color: "#4b4b4b", metalness: 0.65 }), "cannonball");
    group.add(cannonball);
    group.add(mesh(new THREE.SphereGeometry(2.4, 8, 6), new THREE.MeshBasicMaterial({ color: "#9a6942", transparent: true, opacity: 0.16 }), "muzzle-smoke"));
    cannonball.add(mesh(
    ...
    const bolt = mesh(new THREE.CylinderGeometry(0.34, 0.5, 4.4, 8), new THREE.MeshStandardMaterial({ color: "#d39a35", emissive: "#bd3d16", emissiveIntensity: 1.8 }), "flaming-bolt");
    ...
    const bomb = mesh(new THREE.SphereGeometry(0.9, 8, 6), new THREE.MeshStandardMaterial({ color: "#31373a", metalness: 0.5 }), "bomb");
    bomb.position.y = -0.8;
    bomb.add(mesh(
      new THREE.SphereGeometry(2.6, 12, 8),
      new THREE.MeshBasicMaterial({
...
export function updateProjectileTrail(
  ...
  for (let index = trail.puffs.length - 1; index >= 0; index--) {
    ...
    puff.mesh.scale.setScalar(0.4 + progress * 2.6);
